...
ConsoleOutput::PutInt(int number)
{
    ...
    char minus = '-';
    if(number<0){
        WriteFile(writeFileNo, &minus, sizeof(char)); 
        number=-number;    
}
    if(number/10 != 0) 
      PutInt(number/10);
    //PutChar(char('0'+number%10));
    char ch = char('0'+number%10);
    char newline = '\n';
    WriteFile(writeFileNo, &ch, sizeof(char)); 
    if((kernel->machine->ReadRegister(4))==number or kernel->machine->ReadRegister(4)==(-number)){
        WriteFile(writeFileNo, &newline, sizeof(char));
        putBusy == TRUE;
        kernel->interrupt->Schedule(this, ConsoleTime, ConsoleWriteInt);
    } 
      //PutChar('\n');
